...
int main(){
	int size,m2,s2,m3,s3;
	cout<<"\nEnter no. of matrices = ";
	cin>>size;
	MCM sample;
	sample.input(size);
	cout<<"\nDivide and conquer :-";
	cout<<"\nNo. of scalar multiplications = "<<sample.MCM_DC(1,size);
	cout<<"\nDynamic Programming :-";
	cout<<"\ni) Bottom up approach:- ";
	sample.MCM_DP_BT(m2,s2,size);
	cout<<"\nNo. of scalar multiplications = "<<m2;
	cout<<"\nii) Top bottom approach:- ";
	m3 = sample.MCM_DP_TB(1,size);
	cout<<"\nNo. of scalar multiplications = "<<m3;
	cout<<"\nCatalone number = "<<s2;
	cout<<"\nThe optimized solution is ";
	sample.OPT_PAR(1,size);
	return 0;
}
