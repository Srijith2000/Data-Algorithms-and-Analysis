...
class CRSort{
	...
	public:
		...
		void count_sort_radix(int n,int exp,int& count){
			int k=9;
			int C[k+1];
			for(int i=0;i<=k;i++)
				C[i]=0;
...
int main(){
	...
	n=len[0];
	cout<<"1-Counting sort 2-Radix sort\nEnter choice=";
	cin>>choice;
	for(int i=0;i<7;i++){
		n=len[i];
		crs.readInput(n,i);
		count=0;
		if(choice==1)	crs.count_sort(n,k[i],count);
		else if(choice==2) crs.radix_sort(n,count);
		else break;
